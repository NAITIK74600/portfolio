    ...
    >
        ...
        >
            <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className={`${isMobile ? 'w-10 h-10' : 'w-12 h-12 sm:w-14 sm:h-14'} rounded-full overflow-hidden`}
            >
                <video
                    src="/logo-video.mp4"
                    autoPlay
            ...
            >
                <span className="hidden sm:inline">{config.developer.name} | @igxnaitik</span>
...
const ContactButton = ({ isMobile = false, onLinkClick }) => (
    <motion.div
        ...
        onClick={onLinkClick}
    >
        <Link href={"https://github.com/NAITIK74600/Naitik-Raj-portfolio"} target='_blank' className={isMobile ? 'w-full' : ''}>
...
const Header = () => {
    ...
    return (
        <>
            <motion.header
                className="py-4 sm:py-6 md:py-9 z-50 relative text-white"
                style={{ zIndex: 50 }}
